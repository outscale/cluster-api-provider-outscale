...
curl -fs -u admin:$HARBOR_ADMIN_PASSWORD https://$harbor_host/api/v2.0/projects/outscale || \
   curl -XPOST -u admin:$HARBOR_ADMIN_PASSWORD --header "Content-Type: application/json" --data '{"project_name":"outscale", "public":true}' https://$harbor_host/api/v2.0/projects
ret=$?
if [ $ret -ne 0 ]; then
    exit $ret
fi
