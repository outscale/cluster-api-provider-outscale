...
func checkOscSecurityGroupToBeProvisioned(ctx context.Context, oscInfraClusterKey client.ObjectKey, clusterScope *scope.ClusterScope) {
	...
	Eventually(func() error {
		...
		for _, securityGroupSpec := range securityGroupsSpec {
			...
			// Ensure the ResourceMap is populated with the security group ID as the key and name as value
			if _, exists := securityGroupsRef.ResourceMap[securityGroupId]; !exists {
				securityGroupsRef.ResourceMap[securityGroupId] = securityGroupName
				fmt.Fprintf(GinkgoWriter, "Populated SecurityGroupsRef with ID: %s, Name: %s\n", securityGroupId, securityGroupName)
			}
