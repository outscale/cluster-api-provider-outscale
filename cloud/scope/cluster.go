	...
	for _, fd := range fds {
		for _, roles := range [][]infrastructurev1beta1.OscRole{
			...
			case s.SubnetIsPublic(subnet):
				subnet.Name = "Public subnet for " + s.OscCluster.ObjectMeta.Name + "/" + fd
			case s.SubnetHasRole(subnet, infrastructurev1beta1.RoleControlPlane):
				subnet.Name = "Controlplane subnet for " + s.OscCluster.ObjectMeta.Name + "/" + fd
			case s.SubnetHasRole(subnet, infrastructurev1beta1.RoleWorker):
				subnet.Name = "Worker subnet for " + s.OscCluster.ObjectMeta.Name + "/" + fd
			default:
				subnet.Name = "Subnet for " + s.OscCluster.ObjectMeta.Name + "/" + fd
			}
...
func (s *ClusterScope) GetSubnet(name string, role infrastructurev1beta1.OscRole, subregion string) (infrastructurev1beta1.OscSubnet, error) {
	...
	for _, spec := range s.GetSubnets() {
		...
		case spec.SubregionName == subregion:
			return spec, nil
		}
	}
...
func (s *ClusterScope) GetRouteTables() []infrastructurev1beta1.OscRouteTable {
	...
	for _, subnet := range subnets {
		if subnet.SubregionName == "" {
			subnet.SubregionName = s.OscCluster.Spec.Network.SubregionName
		}
...
func (s *ClusterScope) GetSecurityGroups() []infrastructurev1beta1.OscSecurityGroup {
	...
	public := infrastructurev1beta1.OscSecurityGroup{
		Name:        s.GetName() + "-lb",
	...
	node := infrastructurev1beta1.OscSecurityGroup{
		...
		SecurityGroupRules: []infrastructurev1beta1.OscSecurityGroupRule{
			{Flow: "Inbound", IpProtocol: "icmp", FromPortRange: 8, ToPortRange: 8, IpRange: s.GetNet().IpRange},        // BGP
	...
	if !s.OscCluster.Spec.Network.Bastion.Enable {
		return []infrastructurev1beta1.OscSecurityGroup{public, worker, controlplane, node}
	}
	...
	return []infrastructurev1beta1.OscSecurityGroup{public, worker, controlplane, node, bastion}
}
