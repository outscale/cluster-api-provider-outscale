...
//go:generate ../../../bin/mockgen -destination mock_compute/securitygroup_mock.go -package mock_compute -source ./securitygroup.go
...
func (s *Service) CreateSecurityGroup(ctx context.Context, netId string, clusterID string, securityGroupName string, securityGroupDescription string, securityGroupTag string, roles []infrastructurev1beta2.OscRole) (*osc.SecurityGroup, error) {
	...
	err = tag.AddTag(ctx, clusterSecurityGroupRequest, resourceIds, s.tenant.Client())
	if err != nil {
		return nil, err
	}
	if securityGroupTag == "OscK8sMainSG" {
		...
		err := tag.AddTag(ctx, mainSecurityGroupTagRequest, resourceIds, s.tenant.Client())
		if err != nil {
			return nil, err
		}
	}
