...
func ValidateDeviceName(deviceName string) (string, error) {
	switch {
	case strings.Contains(deviceName, "/dev/xvd"):
...
func ValidateVmType(vmType string) (string, error) {
	switch {
	case strings.Contains(vmType, "small") || strings.Contains(vmType, "nano") || strings.Contains(vmType, "micro") || strings.Contains(vmType, "medium") || strings.Contains(vmType, "large") || strings.Contains(vmType, "tina"):
		return vmType, nil
	default:
...
func (s *Service) GetVm(vmId string) (*osc.Vm, error) {
	...
	if !ok {
		return nil, errors.New("Can not gret vm")
	}
	if len(*vms) == 0 {
		return nil, nil
	} else {
