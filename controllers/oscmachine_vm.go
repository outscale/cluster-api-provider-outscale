...
func (r *OscMachineReconciler) reconcileVm(ctx context.Context, clusterScope *scope.ClusterScope, machineScope *scope.MachineScope) (reconcile.Result, error) {
	...
	case fgpu.GetState() == "allocated":
		switch vm.GetState() {
		case "running":
			log.V(3).Info("Stopping VM", "vmId", vm.GetVmId())
			err := r.Cloud.VM(clusterScope.Tenant).StopVm(ctx, vm.GetVmId())
			if err != nil {
		...
		case "stopped":
			...
			return reconcile.Result{RequeueAfter: 30 * time.Second}, nil
		case "stopping":
			log.V(4).Info("Waiting for VM to stop in order to attach fGPU", "vmId", vm.GetVmId(), "fGPUId", fgpu.GetFlexibleGpuId())
			return reconcile.Result{RequeueAfter: 30 * time.Second}, nil
		default:
	...
	default:
		log.V(4).Info("Waiting for fGPU", "vmId", vm.GetVmId(), "fGPUId", fgpu.GetFlexibleGpuId(), "state", fgpu.GetState())
		return reconcile.Result{RequeueAfter: 30 * time.Second}, nil
	}
