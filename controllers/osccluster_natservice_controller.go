...
func (r *OscClusterReconciler) reconcileNatService(ctx context.Context, clusterScope *scope.ClusterScope) (reconcile.Result, error) {
	...
	errs := infrastructurev1beta1.ValidateNatServices(clusterScope.GetNatServices(), clusterScope.GetSubnets(), clusterScope.GetNet())
	if len(errs) > 0 {
		return reconcile.Result{}, errs.ToAggregate()
	}

	for _, natServiceSpec := range clusterScope.GetNatServices() {
		_, err := r.Tracker.getNatService(ctx, natServiceSpec, clusterScope)
		switch {
		case errors.Is(err, ErrNoResourceFound):
		case err != nil:
			return reconcile.Result{}, fmt.Errorf("reconcile natService: %w", err)
		default:
			continue
		}

		publicIpId, err := r.Tracker.allocateIP(ctx, clusterScope.GetNatServiceClientToken(natServiceSpec), clusterScope)
		if err != nil {
			return reconcile.Result{}, fmt.Errorf("reconcile natService: %w", err)
		}

		subnetSpec, err := clusterScope.GetSubnet(natServiceSpec.SubnetName, infrastructurev1beta1.RoleWorker, natServiceSpec.SubregionName)
		if err != nil {
			return reconcile.Result{}, fmt.Errorf("reconcile natService: %w")
		}
		...
		log.V(2).Info("Creating natService")
		natService, err := r.Cloud.NatService(ctx, *clusterScope).CreateNatService(ctx, publicIpId, subnetId,
			clusterScope.GetNatServiceName(natServiceSpec), clusterScope.GetUID())
		if err != nil {
