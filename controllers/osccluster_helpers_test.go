...
func mockCreateSubnet(spec infrastructurev1beta1.OscSubnet, netId, clusterID, subnetId string) mockFunc {
	return func(s *MockCloudServices) {
		s.SubnetMock.EXPECT().
			CreateSubnet(gomock.Any(), gomock.Eq(spec), gomock.Eq(netId), gomock.Eq(clusterID)).
