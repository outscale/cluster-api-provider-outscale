...
import (
	...
	infrastructurev1 "github.com/outscale/cluster-api-provider-outscale/api/v1beta2"
...
func (a *MultiAZAllocator) deployment(m *infrastructurev1.OscMachine) (types.NamespacedName, error) {
...
func (a *MultiAZAllocator) name(m *infrastructurev1.OscMachine) types.NamespacedName {
...
func (a *MultiAZAllocator) AllocateAZ(ctx context.Context, m *infrastructurev1.OscMachine, mode infrastructurev1.SubregionMode, azs []string) (string, error) {
	switch {
	case len(azs) == 0:
		return "", errors.New("no subregions configured")
	case len(azs) == 1:
		return azs[0], nil
	case mode == infrastructurev1.SubregionModeRandom:
...
func (a *MultiAZAllocator) allocateRandomAZ(ctx context.Context, m *infrastructurev1.OscMachine, azs []string) (string, error) {
	az := azs[RandIntN(len(azs))]
	log.FromContext(ctx).V(3).Info("Assigning machine to subregion", "machine", m.Name, "subregion", az)
	return az, nil
}

func (a *MultiAZAllocator) allocateLeastNodeAZ(ctx context.Context, m *infrastructurev1.OscMachine, azs []string) (string, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
	name := a.name(m)
	az, found := a.azs[name]
...
func (a *MultiAZAllocator) refreshWorkers(ctx context.Context, ns string) error {
	var ms infrastructurev1.OscMachineList
	...
	for _, m := range ms.Items {
		...
		if m.Spec.Vm.GetRole() == infrastructurev1.RoleControlPlane {
			continue
		}
		name := a.name(&m)
		a.deployments[deploy] = append(a.deployments[deploy], name)
		a.azs[name] = m.Status.FailureDomain
	}
	return nil
}

func (a *MultiAZAllocator) allocateAZ(ctx context.Context, m *infrastructurev1.OscMachine, azs []string) (string, error) {
