...
func TestReconcileOSCMachine_Create(t *testing.T) {
	tcs := []testcase{
		// Worker node, with 3 reconciliation loops
		{
			...
			mockFuncs: []mockFunc{
				mockImageFoundByName("ubuntu-2004-2004-kubernetes-v1.25.9-2023-04-14", "01234"),
		...
		},
		{
			...
			mockFuncs: []mockFunc{
				...
				mockVmReadCCMTag(false), mockVmSetCCMTag("i-foo", "test-cluster-api-9e1db9c4-bf0a-4583-8999-203ec002c520"),
			},
			machineAsserts: []assertOSCMachineFunc{
		...
		// Volumes
		{
			...
			next: &testcase{
				mockFuncs: []mockFunc{
					mockGetVm("i-foo", "running", "/dev/sdb", "vol-bar"),
					mockVmReadCCMTag(false), mockVmSetCCMTag("i-foo", "test-cluster-api-9e1db9c4-bf0a-4583-8999-203ec002c520"),
				},
				machineAsserts: []assertOSCMachineFunc{
					assertHasFinalizer(),
