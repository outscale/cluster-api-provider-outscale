...
func checkVmLoadBalancerOscAssociateResourceName(machineScope *scope.MachineScope, clusterScope *scope.ClusterScope) error {
	...
	if checkOscAssociate {
		...
		return fmt.Errorf("%s loadBalancer does not exist", vmLoadBalancerName)
	}
...
func reconcileVm(ctx context.Context, clusterScope *scope.ClusterScope, machineScope *scope.MachineScope, vmSvc compute.OscVmInterface, volumeSvc storage.OscVolumeInterface, publicIpSvc security.OscPublicIpInterface, loadBalancerSvc service.OscLoadBalancerInterface, securityGroupSvc security.OscSecurityGroupInterface) (reconcile.Result, error) {
	...
	if vmSpec.ResourceId != "" {
		...
		if err != nil {
			...
			return reconcile.Result{}, fmt.Errorf("%s Can not get vm %s state for OscCluster %s/%s", err, vmID, machineScope.GetNamespace(), machineScope.GetName())
		}
	...
	if vm == nil || vmSpec.ResourceId == "" {
		...
		if err != nil {
			return reconcile.Result{}, fmt.Errorf("%s Can not create vm for OscCluster %s/%s", err, machineScope.GetNamespace(), machineScope.GetName())
		}

		vmID = vm.GetVmId()
		err = vmSvc.CheckVmState(5, 120, "running", vmID)
		if err != nil {
			return reconcile.Result{}, fmt.Errorf("%s Can not get vm %s running for OscCluster %s/%s", err, vmID, machineScope.GetNamespace(), machineScope.GetName())
		}
		machineScope.Info("Vm is running", "vmId", vmID)

		err = volumeSvc.CheckVolumeState(5, 60, "available", volumeId)
		if err != nil {
			return reconcile.Result{}, fmt.Errorf("%s Can not get volume %s available for OscCluster %s/%s", err, volumeId, machineScope.GetNamespace(), machineScope.GetName())
		}
		...
		if err != nil {
			return reconcile.Result{}, fmt.Errorf("%s Can not link volume %s with vm %s for OscCluster %s/%s", err, volumeId, vmID, machineScope.GetNamespace(), machineScope.GetName())
		}

		machineScope.Info("Volume is link", "volumeId", volumeId)
		err = volumeSvc.CheckVolumeState(5, 60, "in-use", volumeId)
		if err != nil {
			return reconcile.Result{}, fmt.Errorf("%s Can not get volume %s in use for OscCluster %s/%s", err, volumeId, machineScope.GetNamespace(), machineScope.GetName())
		}
		...
		if vmSpec.PublicIpName != "" {
			...
			if err != nil {
				return reconcile.Result{}, fmt.Errorf("%s Can not link publicIp  %s with %s for OscCluster %s/%s", err, publicIpId, vmID, machineScope.GetNamespace(), machineScope.GetName())
			}
		...
		if vmSpec.LoadBalancerName != "" {
			...
			if err != nil {
				return reconcile.Result{}, fmt.Errorf("%s Can not link vm %s with loadBalancerName %s for OscCluster %s/%s", err, loadBalancerName, vmID, machineScope.GetNamespace(), machineScope.GetName())
			}
			...
			machineScope.Info("Geet IpProtocol", "IpProtocol", ipProtocol)
			...
			if err != nil {
				return reconcile.Result{}, fmt.Errorf("%s Can not create securityGroupRule for OscCluster %s/%s", err, clusterScope.GetNamespace(), clusterScope.GetName())
			}
...
func reconcileDeleteVm(ctx context.Context, clusterScope *scope.ClusterScope, machineScope *scope.MachineScope, vmSvc compute.OscVmInterface, publicIpSvc security.OscPublicIpInterface, loadBalancerSvc service.OscLoadBalancerInterface, securityGroupSvc security.OscSecurityGroupInterface) (reconcile.Result, error) {
	...
	if vmSpec.PublicIpName != "" {
		...
		if err != nil {
			return reconcile.Result{}, fmt.Errorf("%s Can not unlink publicIp for OscCluster %s/%s", err, machineScope.GetNamespace(), machineScope.GetName())
		}
	}
	if vmSpec.LoadBalancerName != "" {
		err = vmSvc.CheckVmState(5, 60, "running", vmId)
		...
		if err != nil {
			return reconcile.Result{}, fmt.Errorf("%s Can not unlink vm %s with loadBalancerName %s for OscCluster %s/%s", err, loadBalancerName, vmId, machineScope.GetNamespace(), machineScope.GetName())
		}
		...
		err = securityGroupSvc.DeleteSecurityGroupRule(associateSecurityGroupId, flow, ipProtocol, "", securityGroupIds[0], fromPortRange, toPortRange)
		if err != nil {
	...
	if err != nil {
		return reconcile.Result{}, fmt.Errorf("%w Can not delete vm for OscCluster %s/%s", err, machineScope.GetNamespace(), machineScope.GetName())
	}
	return reconcile.Result{}, nil
}
